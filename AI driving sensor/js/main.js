...
let current_gen = (JSON.parse(localStorage.getItem("gen")))
...
inverval_timer = setInterval(function() {
  ...
  cars = cars.filter(c => !c.damaged && c.calculate_distance([{x:best_car.x, y:best_car.y}, {x:best_car.x, y:best_car.y}]) < 1000);
...
function generate_road(last_road, roads, borders, delete_last = true) {
  ...
  let new_road = new Road(...last_road.end, next_type[1] * (750 + Math.random() *500), ...measurements, next_type[0])
  roads.push(new_road);
...
function generate_cars(number, arr) {
  ...
  for (let i=0; i<number; i++) {
    let generated_car = new Car(roads[0].get_lane_center(0), -100, 30, 50, 3, "CPU");
...
function save_best() {
  ...
  localStorage.setItem("gen", current_gen+1);
}
